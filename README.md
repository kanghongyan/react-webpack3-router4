...
1. render function
      ...
      有三个render方法：component, render, children (只能用其中一个 component > render > children)
...
3. path (https://www.npmjs.com/package/path-to-regexp)
 Routes without a path always match.没有path的<Route>总会被匹配到
...
4. exact
   当为true，the path matches the location.pathname。不对结尾是否有/校验
   例如：  path    location.pathname
          /bar    /bar/baz            exact=false        =>  matched!
          /bar    /bar/baz            exact=true         =>  unmatched!
          /bar    /bar/ or /bar       exact=true/false   =>  matched!
          /bar/   /bar/ or /bar       exact=true/false   =>  matched!

5. strict
   当为true，结尾有/的path将会只匹配结尾有/的location.pathname。
   例如：
         path    location.pathname
         /bar/   /bar/ or /bar/xxx/xxx         strict=true  =>  matched!
